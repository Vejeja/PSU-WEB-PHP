...
function print_exec_res($res){
    ...
    foreach($res as $row){
        ...
        <li class="list-group-item flex-sm-fill">'. $row['description'] . '</li>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['set_limit'])){
    ...
    header("location: pagingpage.php");
    exit();
        ...
        <div class="container">
            ...
            <div class="row justify-content-left">
                <?php
                    ...
                    $itm_count = $db->scalar($sqlCount);

                    $sqlSelect = limit_query("SELECT * FROM items", $itm_count, $limit_items);
                    $res = $db->query($sqlSelect);
                    echo print_exec_res($res);
                ?>
            </div>
            <div class="row justify-content-left"> 
                <?php echo paginator_navbar($itm_count, $limit_items); ?>
            </div>
